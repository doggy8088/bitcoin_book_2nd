...
BIP-173 僅對見證指令碼 (P2WPKH 和 P2WSH) 進行編碼。它與非 segwit P2PKH 或 P2SH 指令碼不相容。與傳統的比特幣地址的 Base58 編碼相比，BIP-173 是 Base32 校驗和編碼。BIP-173 地址也稱為 _bech32_ 地址，發音為 "beh-ch thirty two"，暗指使用“BCH”錯誤檢測演算法和 32 字元編碼集。
